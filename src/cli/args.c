...
int tscl_process_args(int argc, tek_sc_os_char **argv, tscl_command **cmds) {
  ...
  if (!tscl_os_strcmp(argv[1], TEK_SC_OS_STR("--help"))) {
    ...
    puts(tsc_gettext(
        ...
        "commands to be used. Specifying <path> as empty string (\"\") implies "
        ...
        "  am run-job <item_id> <manifest_id> <force_verify>  Run/resume a job "
        "for specified item. <manifest_id> can be ID of the manifest to update "
        "to or verify against, or 0 to use ID of the latest available "
        ...
        "  am cancel-job <item_id>                            Cancel a job for "
        "specified item, cleaning up its cache directory and resetting its "
        "state"));
    ...
    puts(tsc_gettext(
        " tek-s3 client:\n"
        "  s3c sync-manifest <url>                            Synchronize "
        "manifest of tek-s3 server at specified URL, i.e. fetch its depot "
        "decryption keys and update list of apps/depots that it can provide "
        "manifest request codes for"));
    puts(
#ifdef TEK_SCB_QR
        tsc_gettext(
            "  s3c signin <type> <url>                            (interactive "
            "mode only) Submit a Steam account to a tek-s3 server at specified "
            "URL. <type> must be either \"credentials\" or \"qr\"")
#else  // def TEK_SCB_QR
        tsc_gettext("  s3c signin <url>                                   "
                    "(interactive mode only) Submit a Steam account to a "
                    "tek-s3 server at specified URL")
